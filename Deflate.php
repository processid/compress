<?php
   // Compression d'un fichier ou d'une chaine en DEFLATE
   // -------------------
   // -- Instanciation --
   // -------------------
   ...
   class Deflate {
            ...
            if ($outputType == self::FILE || $inputType == self::VARIABLE) {
               ...
               if ($this->outputType == self::FILE) {
                  if (!strlen($this->outputFile)) {
                     ...
                     $this->outputFilePointer = fopen($this->outputFile,'w');
      ...
      function compress() {
         if ($this->inputType == self::FILE) {
            stream_filter_append($this->inputFilePointer, 'zlib.deflate', STREAM_FILTER_READ, $this->compressionLevel);
            while (!feof($this->inputFilePointer)) {
               $str = fread($this->inputFilePointer, 8388608);
               if ($this->outputType == self::FILE) {
                  fwrite($this->outputFilePointer, $str);
               } else {
                  $this->outputData .= $str;
               }
            }
            ...
            if ($this->outputType == self::FILE) {
               fwrite($this->outputFilePointer, gzdeflate($this->inputData,$this->compressionLevel));
            } else {
               $this->outputData .= gzdeflate($this->inputData,$this->compressionLevel);
            }
         }
         if ($this->outputType == self::FILE) {
            fflush($this->outputFilePointer);
            fclose($this->outputFilePointer);
            return true;
         } else {
